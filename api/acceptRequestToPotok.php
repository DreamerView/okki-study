<? 
    if(isset($_POST['submit'])) {
        ...
        if($authCheck!==false) {
            if($uuid!==null) {
                $database->start($db,"UPDATE `{$uuid}_users` SET `status`='accept' WHERE `uuid`=? AND `sub_uuid`=?",[$user_uuid,$sub_uuid]);
                $database->start($db,"UPDATE `{$user_uuid}_courses` SET `status`='accept' WHERE `uuid`=? AND `sub_uuid`=?",[$uuid,$sub_uuid]);
                if($lazy==='yes') {
                    echo "?location=admin_potok_add&&uuid={$uuid}";
                } else {
                    header("Location: {$mainURL}/?location=admin_potok_add&&uuid={$uuid}");
                }
                exit();
            }
        }
