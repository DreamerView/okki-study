<? 
    ...
    if($uuid!==null) {
        ...
        if($check===false) $uuid=null;
            ...
            $select = $database->fetchAll($db,"SELECT `uuid`,`status`,`time` FROM `{$uuid}_users` WHERE `status`=? AND `sub_uuid`=?",['waiting',$potok_uuid]);
...
<div class="mt-5 block_animation">
    <h1>Курс потоктың заявкалары</h1>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="d-flex gap-2">
                    <form data-form method="POST" action="api/acceptRequestToPotok.php">
                        ...
                        <button title="Қосу" class="ms-2 btn btn-primary" type="submit" name="submit"><i class="bi bi-person-fill-add"></i></button>
                    </form>
